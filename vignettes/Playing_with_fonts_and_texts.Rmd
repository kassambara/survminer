...
ggsurvplot(fit, risk.table = TRUE, ncensor.plot = TRUE)
```
...
- Distinct between `main` and `submain` for the `curve_plot` and `risk.table.title` and `risk.table.subtitle` for the `table` / `ncens.plot.title` and `ncens.plot.subtitle` for the `ncens`.
