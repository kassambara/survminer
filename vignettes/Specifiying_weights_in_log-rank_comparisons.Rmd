...
Regular Log-rank comparison uses $w_{t_i} = 1$ but many modifications to that approach have been proposed. The most popular modifications, called weighted Log-rank tests, are
...
> Note: the Fleming-Harrington weight uses $S(t^-)$ which is the left-continuous Kaplan-Meier estimate (the value just before time $t$).
...
# The `weighted_logrank()` function

The examples above annotate a single weighted test on a plot. When you want the
tests themselves -- the statistic, its degrees of freedom and the p-value -- as a
tidy table, use `weighted_logrank()`. It takes vectors of `rho` and `gamma`, so
you can compute a whole family of Fleming-Harrington G(rho, gamma) tests in one
call. The classic three are FH(0, 0) (the standard log-rank), FH(1, 0)
(early-emphasis, the Peto-Peto / Prentice weighting), and FH(0, 1)
(late-emphasis):
...
Each row is one test with its weight, `rho`, `gamma`, chi-square `statistic`, `df`
and `p.value`. Here the early-emphasis FH(1, 0) test gives the smallest p-value
and the late-emphasis FH(0, 1) test the largest, which tells you the two survival
curves separate more in early follow-up than late -- the kind of shape a single
log-rank p-value hides.

The same arbitrary `(rho, gamma)` are available inside `ggsurvplot()` through
`log.rank.weights = "FH"` with `rho =` and `gamma =`, and `surv_pvalue()` now
returns the test `statistic` and `df` alongside the p-value:

```{r}
surv_pvalue(fit, data = lung, method = "FH", rho = 0, gamma = 1)
```
