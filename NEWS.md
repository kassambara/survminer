...
- Fix `ggsurvplot(fit, facet.by = ...)` silently dropping the fit's case `weights` when the `facet.by` variables are not all in the survfit formula. In that case the curves are refit to add the faceting variables, and the refit ignored `weights`, so the faceted curves and the `surv.median.line` were computed **unweighted** — silently wrong. The recovered weight vector is now forwarded to the refit, so weighted faceting draws the correct (weighted) curves and medians. Unweighted fits are unchanged. Two caveats, each now made explicit with a warning: the per-panel p-value uses `survival::survdiff()`, which has no case-weights argument, so it remains unweighted; and weights passed as an out-of-scope object (e.g. `weights = df$w`) rather than a bare column (`weights = w`) cannot be recovered for the refit and fall back to unweighted. Reported by @goncosta (#556).
