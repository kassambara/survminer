...
- New "Absolute effect measures" vignette covering the survival summaries a reviewer now expects beyond the log-rank p-value and hazard ratio: restricted mean survival time (`ggrmst()` / `ggrmst_difference()`, with the area-to-tau shaded and the difference annotated), milestone (fixed-time) survival with the between-arm difference (`ggmilestone()`), and landmark analysis for immortal-time bias (`gglandmark()`).

- New "Interrogating a Cox model" vignette on the four questions a reviewer asks of a fitted Cox model: what each covariate does (`ggforest()`, with a forest-reading primer), whether the treatment effect is the same in every subgroup (`ggforest_subgroup()`, with the interaction test), whether it survives adjustment (`ggforest_models()`, crude vs. adjusted), and whether the proportional-hazards assumption holds (`ggcoxnph()`, a four-panel diagnostic worked through on a genuinely non-proportional example).

- New "Competing risks" vignette on why 1 − Kaplan-Meier overestimates incidence when events compete, the cumulative incidence function drawn with `ggcompetingrisks()` (with confidence bands), the naive-KM overestimate made visible, Gray's test between groups, and a number-at-risk table -- worked through on the `mgus2` progression-vs-death example.

- The "Specifying weights in Log-rank comparisons" vignette now has a section on the `weighted_logrank()` function, showing a whole Fleming-Harrington family (early-emphasis FH(1, 0), standard FH(0, 0) and late-emphasis FH(0, 1)) computed in one call as a tidy table with the test statistic and degrees of freedom.
