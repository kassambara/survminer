...
- Fix `ggcompetingrisks()` (multi-state / `survfitms` competing-risks fits) sorting its facet panels alphabetically by strata name instead of following the model's strata order. The internal `strata` column is now an ordered factor, so panels keep `names(fit$strata)` order. Fits whose strata order already is alphabetical are unchanged. Contributed by @muschellij2 (#470).
